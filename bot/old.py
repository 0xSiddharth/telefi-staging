...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
	...
	if update.message.chat.username.lower() in db:
		...
		await update.message.reply_text(
			"@%s has requested you pay %s!" % (db[update.message.chat.username.lower()], db[update.message.chat.username.lower()]),
			reply_markup=ReplyKeyboardMarkup.from_button(
				KeyboardButton(
					...
					web_app=WebAppInfo(url="https://telefi-staging.vercel.app?send=1&toaddr=%s&amount=%s" % (addresses[db[update.message.chat.username.lower()]], str(db[db[update.message.chat.username.lower()]]/eth_price).replace('.','_'))),
					)
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	try:
		...
		user_handle = strip_url(text)
		print(user_handle)
		db[user_handle.lower()] = update.message.chat.username.lower()
		prev_users[update.message.chat.username.lower()] = text.lower()
...
async def balance_check(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	await update.message.reply_text(message_start + message)
